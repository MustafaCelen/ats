...
import { Wallet, RefreshCw, CheckCircle2, XCircle, Loader2, AlertTriangle, TrendingUp, Users, Clock, Upload } from "lucide-react";
...
type MainTab = "debts" | "report" | "unmatched" | "preview";
...
export default function FonzipPreview() {
  ...
  const unmatchedBadge = unmatched?.length ?? 0;
  const hasData = (stats?.total ?? 0) > 0 || (userFinancials?.length ?? 0) > 0;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              Fonzip Entegrasyonu
              {status && (
                <Badge variant={status.configured ? "default" : "destructive"} className="gap-1 ml-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(excelImporting || excelImportStatus?.running) && (
              <Button onClick={() => resetImportMutation.mutate()} size="sm" variant="destructive">
                İptal / Sıfırla
              </Button>
            )}
            <Button
              ...
              {(excelImporting || excelImportStatus?.running)
                ? (() => {
                    ...
                    if (p && p.total > 0) {
                      ...
                      return <><Loader2 className="h-4 w-4 animate-spin" />{pct}% ({p.current}/{p.total})</>;
                    }
                    return <><Loader2 className="h-4 w-4 animate-spin" />İşleniyor...</>;
                  })()
            ...
            >
              {isUsersSyncRunning
                ? <><Loader2 className="h-4 w-4 animate-spin" />Borçlar sync...</>
                : <><Users className="h-4 w-4" />Toplam Borç Sync</>}
            </Button>
            <Button
              className="gap-1.5"
            ...
            >
              {isSyncRunning
                ? <><Loader2 className="h-4 w-4 animate-spin" />Detay sync...</>
                : <><RefreshCw className="h-4 w-4" />Detaylı Sync</>}
            </Button>
          </div>
        </div>
        ...
        {hasData && (
          <>
            ...
            <div className="flex items-center gap-1 border-b border-border">
              {([
                { key: "debts" as const, label: "Borçlu Danışmanlar", badge: userFinancials?.length ?? 0 },
                { key: "report" as const, label: "Aidat Raporu (Detay)" },
                { key: "unmatched" as const, label: "Eşleşmeyenler", badge: unmatchedBadge },
                { key: "preview" as const, label: "Ham Veri" },
              ]).map((t) => (
                <button
                  key={t.key}
              ...
              <div className="space-y-4">
                ...
                {preview && (
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <Card>
                      <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Üyeler</CardTitle></CardHeader>
                      <CardContent><JsonViewer data={preview.users} /></CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Aidatlar</CardTitle></CardHeader>
                      <CardContent><JsonViewer data={preview.debts} /></CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Bağışlar</CardTitle></CardHeader>
                      <CardContent><JsonViewer data={preview.donations} /></CardContent>
                    </Card>
                  </div>
                )}
